...
# Copyright (c) 2021 - 2024 Cloudera, Inc.
# All Rights Reserved.
#
# This file is confidential and proprietary to Cloudera, Inc. and may not be used, viewed,
# disclosed, or reproduced without Cloudera's express written permission.
...
subcommand_usage() {
 ...
 add-spark-config-in-virtual-cluster    Add/update a spark config in a CDE Virtual Cluster  -h <virtual-cluster-host> -c <spark configs> --gang-scheduling <gang scheduling>
             ...
             edit-cluster-autoscaler    Modify the duration of node scaling down to         --scale-down-delay-after-add <delay after add>         --scale-down-delay-after-delete <delay after delete>
                                        ...
                                        scaling down process.                               --unremovable-node-recheck-timeout <recheck timeout>   -i <interactive menu>
   update-control-plane-certificates    Adds the updated control plane certificates into a  -h <virtual-cluster-host> -n <control plane namespace>
                                        CDE Service." 1>&2
}
...
auto_create_certs() {
    ...
    # keyUsage and extendedKeyUsage are to restrain the use of certificate. Removing the restrictions in case of spark-connect certificates.
...
CN = ${DOMAIN_NAME}
[v3_req]$(if [ "${SCOPE}" != "dex-app-sc" ]; then echo "
keyUsage = nonRepudiation, digitalSignature, keyEncipherment
extendedKeyUsage = serverAuth"; fi)
...
EOF
    ...
    # Append all the certs in cacerts.pem
    run_cmd "touch cacerts.pem"
    run_cmd "cat $CERT_PATH >> cacerts.pem"
}
...
__setup_ingress() {
    ...
    do
        case ${options} in
            (h)
                ...
                # This is needed when a new VC is created under an upgraded CDE service. The ingresses in of new VC use the original CDE service
                # Id. The patching of TLS certs should be applied to the new namespace where the new CDE service actually exists and this namespace
                # is different from what we extract from ingress host passed in the script with -h argument.
                if [ "${SCOPE}" == "dex-app" ]; then
                  echo "[Upgrade] checking if the cluster-id ${BASE_ID} extracted from ${INGRESS_HOST} is same as the on in the dex-app namespace"
                  CLUSTER_ID_FROM_NS=$(run_kubectl_cmd "kubectl get namespace dex-app-${VC_ID} -o jsonpath='{.metadata.labels.cdp\.cloudera\.com/cde-service}' | cut -d - -f 2")
                  if [ "${CLUSTER_ID_FROM_NS}" != "${BASE_ID}" ]; then
                    echo "[Upgrade] found different cluster-id ${CLUSTER_ID_FROM_NS} for dex-app-${VC_ID}. Will use namespace dex-base-${CLUSTER_ID_FROM_NS} for patching ingresses"
                    K8S_NAMESPACE="dex-base-${CLUSTER_ID_FROM_NS}"
                  fi
                fi
                ...
                elif [ "${SCOPE}" == "dex-app-sc" ]; then
                    APP_NAMESPACE="dex-app-${VC_ID}"
	            INGRESS_PREFIX="${APP_NAMESPACE}-sc"
                    CONFIGMAP_NAME="${APP_NAMESPACE}-api-cm"
                    CONFIGMAP_DATA=$(kubectl get configmap "$CONFIGMAP_NAME" -n "$APP_NAMESPACE" -o jsonpath='{.data.dex\.yaml}') || { echo "Error getting ConfigMap data"; exit 1; }
                    # Extract the sparkVersion and appTier
                    SPARK_VERSION=$(echo "$CONFIGMAP_DATA" | grep 'sparkVersion' | awk '{print $2}' | sed 's/^"\(.*\)"$/\1/' | head -n 1) || { echo "Error getting Spark Version"; exit 1; }
                    APP_TIER=$(echo "$CONFIGMAP_DATA" | grep 'appTier' | awk '{print $2}' | sed 's/^"\(.*\)"$/\1/') || { echo "Error getting appTier"; exit 1; }
                    # tier-2 is same as ALLP, we have included both for backward compatibility
                    if ! ([ "$SPARK_VERSION" == "3.4.1" ] && ( [ "$APP_TIER" == "tier-2" ] || [ "$APP_TIER" == "ALLP" ] )); then
                      log_info "Skipping spark-connect secret generation for ${K8S_NAMESPACE}.";
                      return 0
                    fi
                    # Extract the sparkConnectUrl which is INGRESS HOST
                    INGRESS_HOST=$(echo "$CONFIGMAP_DATA" | grep 'sparkConnectUrl' | awk '{print $2}' | sed 's/^"\(.*\)"$/\1/') || { echo "Error getting Spark Connect Url"; }
                    if [ -z "$INGRESS_HOST" ]; then
                      echo "INGRESS_HOST found empty in case of dex-app-sc, Skipping..."
                      return 0
                    fi
                fi
    ...
    # Attempt to remove passphrase from the key
    KEY_PATH_WITHOUT_PASSPHRASE="${CERT_DIR}/$(LC_CTYPE=C tr -dc 'a-zA-Z0-9' < /dev/urandom | head -c 5)_script.pem"
    # This line will try to remove passphrase from the key, if it's not pass phrase protected it will simply copy the same.
    run_cmd "openssl pkey -in ${KEY_PATH} -out ${KEY_PATH_WITHOUT_PASSPHRASE}"

    # Check the exit status of the openssl pkey command
    if [ $? -ne 0 ]; then
        echo "Error: Failed to remove passphrase from the key"
        run_cmd "rm -f ${KEY_PATH_WITHOUT_PASSPHRASE}"
        exit 1
    fi

    log_info "Creating secrets out of TLS certs"
    run_kubectl_cmd "kubectl create secret tls ${SECRET_NAME} --cert=${CERT_PATH} --key=${KEY_PATH_WITHOUT_PASSPHRASE} -o yaml --dry-run | kubectl apply -f - -n ${K8S_NAMESPACE}"
...
subcommand_update_control_plane_certificates() {

    unset OPTIND OPTARG options
    while getopts "h:n:" options
    do
        case ${options} in
            (h)
                VC_HOST=${OPTARG}
    ...
    if [ -z "${VC_HOST}" ]; then
        log_error "Missing VC host value. Use -h to specify the host.."
        exit 1
    fi

    if [ -z "${CP_NAMESPACE}" ]; then
        log_error "Missing control plane namespace value. Use -n to specify it.."
        exit 1
    fi

    BASE_ID=`echo ${VC_HOST} | cut -d. -f 2 | sed 's/cde-//g'`

    DEX_BASE_K8S_NAMESPACE="dex-base-${BASE_ID}"
    log_info "dex-base namespace is ${DEX_BASE_K8S_NAMESPACE}"

    PATCH_DIR="${SCRATCH_SPACE}/patches"
    run_cmd "mkdir -p ${PATCH_DIR}"
    # Copy and patch the jks cacert used by configs-manager
    log_info "updating certs in ${DEX_BASE_K8S_NAMESPACE}"
    cacerts_obj=$(run_kubectl_cmd "kubectl get configmap cdp-pvc-truststore -n ${CP_NAMESPACE} -o=jsonpath='{.binaryData.JKS}'")
    echo "{ \"binaryData\": { \"cacerts\": \"${cacerts_obj}\" } }" > ${PATCH_DIR}/cacerts_patch.json
    run_kubectl_cmd "kubectl patch configmap private-cloud-ca-certs --type merge --patch-file ${PATCH_DIR}/cacerts_patch.json -n ${DEX_BASE_K8S_NAMESPACE}"
    run_kubectl_cmd "kubectl rollout restart deployment dex-base-configs-manager -n ${DEX_BASE_K8S_NAMESPACE}"

    pem_obj=$(run_kubectl_cmd "kubectl get configmap cdp-pvc-truststore -n ${CP_NAMESPACE} -o=jsonpath='{.data.PEM}'")

    # check if we use new configmap for knox, else update the old secret to handle for cluster before 1.5.4 sp1
    if run_kubectl_cmd "kubectl get cm knox-cdp-cert -n ${DEX_BASE_K8S_NAMESPACE}" &> /dev/null; then
      echo "{ \"data\": { \"cdp.crt\": \"${pem_obj}\" } }" > ${PATCH_DIR}/pem_patch_multiline.json
      sed '$!s/$/\\n/' ${PATCH_DIR}/pem_patch_multiline.json | tr -d '\n' > ${PATCH_DIR}/pem_patch.json
      run_kubectl_cmd "kubectl patch configmap knox-cdp-cert --type merge --patch-file ${PATCH_DIR}/pem_patch.json -n ${DEX_BASE_K8S_NAMESPACE}"
    else
      pem_obj_b64=$(echo "${pem_obj}" | base64)
      echo "{ \"data\": { \"cdp.crt\": \"${pem_obj_b64}\" } }" > ${PATCH_DIR}/pem_b64_patch.json
      run_kubectl_cmd "kubectl patch secret dex-base-knox-keys --type merge --patch-file ${PATCH_DIR}/pem_b64_patch.json -n ${DEX_BASE_K8S_NAMESPACE}"
    fi
    run_kubectl_cmd "kubectl rollout restart deployment dex-base-knox -n ${DEX_BASE_K8S_NAMESPACE}"
    run_cmd "rm -rf ${PATCH_DIR}"
    log_info "The knox and configs-manager pods have been restarted. Wait for all pods to become Running.
     You can use the following commands to see the status of the pods:
        kubectl get pods -n ${DEX_BASE_K8S_NAMESPACE}
        "
}
...
main() {
    ...
    case $subcommand in
        ...
        edit-cluster-autoscaler)
            subcommand_edit_cluster_autoscaler "$@"
            ;;
        update-control-plane-certificates)
            subcommand_update_control_plane_certificates "$@"
            ;;
        *)
            # unknown option
